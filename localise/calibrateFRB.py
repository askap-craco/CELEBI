...
def flag_data(data, flagfile: str, shadow: "list[str]") -> None:
    ...
    print("Flagging with file " + flagfile)
...
def run_split(data, outfname: str, sourcename: str) -> None:
    ...
    run("rm -f " + tempoutfits)
    vlbatasks.writedata(data, tempoutfits, True)
    run("mv " + tempoutfits + " " + outfname + ".unavg")
...
def get_peak_pixel(tcleanargs):
    ...
    casaout.close()
    run("python {0}".format(scriptname))
