...
class CalcFile:
    ...
    def addtel(self, cname, vkey=None, index=None, default=None):
        ...
        assert (
            value is not None
        ), f"Unknown value for ant {self.telno} key {cname} {vkey}"
        if index is not None:
            bits = value.replace("[", "").replace("]", "").split(",")
            value = bits[index].strip()

        cardkey = f"TELESCOPE {self.itel} {cname}"
        self += (cardkey, value)
...
class Poly:
    ...
    @property
    def mjdfull(self):
        return self.mjd + self.sec / 86400.0
    ...
    def __str__(self):
        return f"POLY ID={self.polyid} mjd={self.mjd} sec={self.sec} ants={str(list(self.source0antpolys.keys()))}"
...
class Scan:
    ...
    def last_mjd(self):
        return max(
            self.polys, key=lambda p: p.mjdfull
        ).mjdfull  # COuld add interval but don't care right at the moment
    ...
    def get_poly(self, mjd):
        ...
        if len(after_polys) == 0:
            ...
            warnings.warn(
                f"Past last polynomial. mjd={mjd} last mjd={the_poly.mjdfull}"
            )
        else:
    ...
    def eval_src0_poly(self, mjd):
        ...
        secs = (mjd - poly.mjdfull) * 86400.0

        if secs < 0.0:
            warnings.warn(
                f"ERR Dodgey offset. mjd={mjd} polymjd ={poly.mjdfull} secoffset={secs}"
            )
        assert secs >= 0
        ...
        for ant, polys in poly.source0antpolys.items():
            ...
            if abs(elevation) < 2:
                warnings.warn(
                    f"ERR: Evaluating geoemtry at very low elevation. Be careful! Elevation={elevation} mjd={mjd} poly={polys}"
                )
...
class ResultsFile(OrderedDict):
    def __init__(self, fname):
        ...
        for itel in range(self.num_telsecopes):
            self.telnames.append(self[f"TELESCOPE {itel} NAME"])
...
def plot_polys_range(rfile, mjdstart, tmax):
    ...
    :tmax: seconds
    """
    toff = np.arange(tmax, step=60.0)
    mjds = toff / 86400.0 + mjdstart
    values = []
    ...
    for ia1, a1 in enumerate(rfile.telnames):
        for ia2, a2 in enumerate(rfile.telnames[ia1:]):
            ...
            delays = [
                val[a1]["DELAY (us)"] - val[a2]["DELAY (us)"] for val in values
            ]
            u, v, w, secoffs, delays = list(
                map(np.array, (u, v, w, secoffs, delays))
            )
            if np.any(abs(u) > 1e5):
    ...
    ax5.set_xlabel(f"Seconds from MJD {mjdstart:.5f}")
...
def _main():
    ...
    parser.add_argument(
        "--nhrs", type=float, help="Number of hours to plot", default=1.0
    )
    ...
    print(
        f"File {values.files[0]} starts at mjd {mjdstart} and ends at {mjdend}"
    )
