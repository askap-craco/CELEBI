...
class VcraftMux(object):
    ...
    def __init__(self, vcraft_files, delays=None):
        ...
        self.sample_offsets = self.start_frameid - np.array([f.start_frameid for f in self._files]) + np.array([int(delays.get(os.path.basename(f.fname+'.hdr'), 0)) for f in self._files]) # temporary
        ...
        beam_ra = np.array(list(map(float, self.allhdr('BEAM_RA')))).mean() # TODO: make craft_vdump write same BEAM_RA for all card/fpgas
