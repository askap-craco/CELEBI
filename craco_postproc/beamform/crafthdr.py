...
class DadaHeader(OrderedDict):
    ...
    def str(self, check=True):
        ...
        if check:
            ...
            assert (
                len(s) <= hsize
            ), f"Header size is {len(s)} but HDR_SIZE={hsize}"
...
def default_header(
    ...
    hdr += (
        "DORDER",
        "TFBP",
        "Data ordering. Last is fastest. T=Time, B=Beam, P=Polarsation, F=Freq",
    )
    ...
    hdr += (
        "BEAM_RA",
        ",".join(map(str, ras)),
        "Right Ascensions of all beams (J2000 - degrees)",
    )
    hdr += (
        "BEAM_DEC",
        ",".join(map(str, decs)),
        "Declinations of all beams (J2000 - degrees)",
    )
    hdr += (
        "BEAM_POL",
        ",".join(map(str, beam_pol)),
        "Polarisation mnemonic for beams. e.g. XX or YY",
    )
    ...
    hdr += (
        "SUM_WEIGHTS",
        ",".join(map(str, sum_weights)),
        "Weights of each antenna going into an incoherent sum",
    )
    hdr += ("ANTNAME", "AK%02d" % antno, "Antenna name")
    hdr += (
        "INT_CYCLES",
        int_cycles,
        "Number of integrations per beamformer packet",
    )
    hdr += ("INT_TIME", int_time, "Number of samples per integration")
    hdr += (
        "OBS_OVERLAP",
        0,
        "The amount by which neighbouring fiels overlap - See PSRDADA",
    )
    hdr += (
        "OBS_OFFSET",
        0,
        "The number of byte from the start of the observation - See PSRDADA",
    )
    ...
    hdr += (
        "FILE_SIZE",
        file_size,
        "Number bytes to write to a file before opening another one",
    )

    hdr += ("NUM_BEAMFORMERS", len(bfaddr), "Number of beamformers")
    # chanmap = np.arange(nchan)

    for ibf, (bf, chanmap, freqmap) in enumerate(
        zip(bfaddr, chanmaps, freqmaps)
    ):
        # freqmap = chanmap*bw + fstart + nchan*bw*ibf
        hdr += (
            "BEAMFORMER%d_ADDR" % ibf,
            bf,
            "Address of beamformer %d" % ibf,
        )
        hdr += (
            "BEAMFORMER%d_CHANMAP" % ibf,
            ",".join(map(str, chanmap)),
            "Channel map for beamformerr %d" % ibf,
        )
        hdr += (
            "BEAMFORMER%d_FREQMAP" % ibf,
            ",".join(map(str, freqmap)),
            "Frequency map for beamformerr %d" % ibf,
        )
...
def _main():
    ...
    parser = ArgumentParser(
        description="Captures CRAFT autocorrelation spectra and writes to SIGPROC file"
    )
    parser.add_argument(
        "-v",
        "--verbose",
    ...
    parser.add_argument(
        "-a", "--ant", help="Antenna number", type=int, default=-1
    )
    parser.add_argument(
        "-c", "--card", help="Card number", type=int, default=-1
    )
    parser.add_argument("-o", "--output", help="Output file name")
    parser.add_argument(
        "-i",
        "--int-time",
        help="Integration time (1-65535)",
        default=1000,
        type=int,
    )
    parser.add_argument(
        "-s",
        "--int-cycles",
        help="Number of cycles to combine (1-7)",
        default=3,
        type=int,
    )
    parser.add_argument(
        "-b",
        "--beam-number",
        help="Beam number to save voltages for (0-35). Unspecified for all beams",
        default=None,
    )
    parser.add_argument(
        "-m",
        "--buffer-mode",
        help="Buffer number of bits",
        choices=[16, 8, 4, 1],
        type=int,
        default=16,
    )
    ...
    assert (
        int_time >= 1 and int_time <= 65535
    ), f"Invalid integration time {int_time}"

    int_cycles = int(values.int_cycles)
    assert (
        int_cycles >= 1 and int_cycles <= 7
    ), f"Invalid int cycles {int_cycles}"

    samp_rate = 32.0 / 27.0 * 1e6  # samples/sec
    tsamp = float(int_time) / samp_rate

    bufmode_map = {16: 0, 8: 1, 4: 2, 1: 3}
    assert values.buffer_mode in list(
        bufmode_map.keys()
    ), "Invalid buffer mode"
    ...
    if values.beam_number is None:
        ...
        assert (
            beam_number >= 0 and beam_number < 36
        ), f"Invalid beam number to save {beam_number}"
