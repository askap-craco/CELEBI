...
class aipscor:
    def __init__(
        ...
        self.polfringfile = (
            None  # self.fringfile.replace('delays.sn.txt','xpolfring_xpol.sn')
        )
    ...
    def get_basic_info(self):
        ...
        if self.specific_frb == "190608":  # exclude ak13, ak19, ak20, ak28
            for ak in [13, 19, 20, 28]:
                all_vfiles = list(
                    set(all_vfiles)
                    - set(glob.glob(self.vcraft_dr + f"/*ak{ak}*/*/*.vcraft"))
                )
            for f in all_vfiles:
        ...
        if self.npol == 1:
            ...
            if ntotf != len(all_vfiles):
                print(
                    (
                        "ERROR, file lost in the sorting process. expected: ",
                        len(all_vfiles),
                        ", received: ",
                        ntotf,
                    )
            ...
            for f in range(6):
                ...
                with open(
                    ...
                    for line in fl:
                        if "FREQS" in line:
                            freqs = (
                                np.array(line.split()[1].split(",")).astype(
                                    int
                                )
                                + freq_offset
                            )
    ...
    def map_an_name(self):  # , an_ind):
        ...
        if self.specific_frb == "190608":  # exclude ak13, ak19, ak20, ak28
            an_names = list(set(an_names) - {"ak13", "ak19", "ak20", "ak28"})
    ...
    def map_anname_nosta(
        self,
    ):  # TODO! later make every get_delay/phase functions check the annames instead of iant and read in the correct calibration solutions from bandpass/fring/selfcal.
        an_dict = {}
        # hdul = fits.open(fits)
    ...
    def get_hwdelay(self, an_ind, pol, an_check=0, incards=False):
        ...
        if not incards:
            delays_offset = np.full(
                (self.nfreq),
                np.nan,
                dtype=[("frequency", int), ("hwdelay", int)],
            )
        else:
            hwdelays = {}
        ...
        for c in range(1, 8):
            for f in range(6):
                card_name = "c" + str(c) + "_f" + str(f)
                with open(
                    self.input_dr + "/" + card_name + "/craftfrb.input"
                ) as fl:
                    if (
                        an_check
                    ):  # Quick check to see if antenna index and antenna name match
                        for i in range(self.nant):
                            fl.seek(0)
                            fl.seek(
                                fl.read().find("TELESCOPE NAME " + str(i)), 0
                            )
                            input_an_name = str(fl.readline().split()[-1])
                            if input_an_name != self.an_name[i]:
                                print(
                                    "WARNING: telescope name and index dont match"
                                )

                    # read in hw delays

                    vcraft_fn = (
                        self.an_name[an_ind] + "_" + card_name + ".vcraft"
                    )
                    ...
                    for nu in range(len(freqs)):
                        ...
                        if where != nu:
                            print(
                                "WARNING: the frequency order is different from the vcraft file"
                            )

                    fl.seek(0)
                    fl.seek(
                        fl.read().find("TELESCOPE INDEX:    " + str(an_ind))
                    )
                    bool_ypol_yet = False
                    for nu in range(len(freqs)):
                        bool_continue = True
    ...
    def get_delay_clock(self, an_ind, pol, an_check=0):
        ...
        f = 0
        card_name = "c" + str(c) + "_f" + str(f)
        with open(self.input_dr + "/" + card_name + "/craftfrb.input") as fl:
            if (
                an_check
            ):  # Quick check to see if antenna index and antenna name match
                for i in range(self.nant):
    ...
    def get_delay_fring(self, an_ind, pol):
        ...
        with open(self.fringfile) as fl:
            fl.seek(0)
            ...
            while not bool_record:
                ...
                if self.specific_frb != "190608":
                    if data.split()[0] == str(an_ind + 1):
                        bool_record = True
                        delays_fring = float(
                            data.split()[delay_ind]
                        )  # FRING fine time delay
                else:
                    temp = data.split()[0]
    ...
    def get_delay_polfring(self, an_ind, pol):
        ...
        with open(self.polfringfile) as fl:
            fl.seek(0)
            fl.seek(fl.read().find(start), 0)
            ...
            while not bool_record:
                ...
                if data.split()[0] == str(an_ind + 1):
                    bool_record = True
                    if 1:
                        delays_polfring = float(
                            data.split()[delay_ind]
                        )  # Polarization FRING fine time delay
        ...
        with open(self.fringfile) as fl:
            ...
            while not bool_record:
                ...
                if self.specific_frb != "190608":
                    if data.split()[0] == str(an_ind + 1):
                        bool_record = True
                        phase_fring_real = float(
                            data.split()[delay_ind]
                        )  # FRING real phase
                        phase_fring_imag = float(
                            data.split()[delay_ind + 1]
                        )  # FRING imag phase
                else:
                    temp = data.split()[0]
                    if an_ind < 18 and temp == str(an_ind + 1):
            ...
            if (abs(phase_fring) - 1) > 1e-3:
                print(
                    "WARNING: amplitude of FRING phase is not 1 but "
                    + str(abs(phase_fring))
                )
        return phase_fring
                ...
                if self.specific_frb != "190608":
                    if data.split()[0] == str(an_ind + 1):
                        ...
                        for chan in range(nfreq):
                            phase_bp_real[chan] = float(
                                data.split()[delay_ind]
                            )  # bandpass real phase
                            phase_bp_imag[chan] = float(
                                data.split()[delay_ind + 1]
                            )  # bandpass imag phase
                            data = fl.readline()
                else:
