...
class PlotOut:
    def __init__(self, corr):
        ...
        self.corr = corr
        self.delayout = open(
            corr.values.outfile.replace(".fits", "") + ".delays.parset", "w"
        )
...
class AntennaSource:
    ...
    def do_f(self, corr):
        ...
        logging.debug(
            "F %s sample delays: frame: %f geo %f fixed %f total %f whole: %d frac: %f nsamp: %d phase=%f deg",
            self.antname,
            framediff_samp,
            geom_delay_samp,
            fixed_delay_samp,
            ...
            frac_delay_samp,
            nsamp,
            360.0 * frac_delay_us * corr.f0,
        )
        logging.debug(
            "F %s us delays: frame: %f geo %f fixed %f total %f whole: %d frac: %f nsamp: %d phase=%f deg rate=%e",
            self.antname,
            framediff_us,
            geom_delay_us,
            fixed_delay_us,
        ...
        for c in range(corr.ncoarse_chan):
            ...
            logging.debug(
                "PHASOR %s[%s] chan=%s freq=%sfixed=%f us geom=%f us delta_t %s us coff*fixed = %f deg coff*geom = %f deg",
                self.antname,
                self.ia,
                c,
                cfreq,
                ...
                geom_delay_us,
                delta_t,
                cfreq * fixed_delay_us * 360.0,
                cfreq * geom_delay_us * 360.0,
            )
    ...
    def do_f_tab(self, corr, iant):
        ...
        print(
            basestr.format("np.mean(deom_delays_us)", np.mean(geom_delays_us))
        )
...
class Correlator:
    ...
    def get_calc_results(self, mjd):
        ...
        res = self.calcresults.scans[0].eval_src0_poly(
            mjd
        )  # Calcresults is a ResultsFile
            ...
            for p2 in range(self.npol_in):
                ...
                try:
                    for c in range(self.nfine_chan):
                        ...
                        if self.sideband == -1:
                            xx[c, pout] = (
                                np.vdot(d2[:ntimesi, c], d1[rfidelay:, c])
                                / ntimes
                            )
                        else:  # take complex conjugate if inverted
                            xx[c, pout] = (
                                np.vdot(d1[:ntimesi, c], d2[rfidelay:, c])
                                / ntimes
                            )
...
def _main():
    ...
    parser.add_argument(
        "--tab",
        help="Do tied-array beamforming",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "--ics",
        help="Do incoherent beamforming",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    if 0:  # values.mirsolutions is not None:
        solutions = MiriadGainSolutions(
            values.mirsolutions, values, values.aips_c, "y"
        )
        if 0:
            solutions.plot()
            pylab.show()
