...
parser.add_argument(
    ...
    type=float,
    default=-99999,
    help="The time difference between the VCRAFT and snoopy log arrival times for the pulse, including geometric delay, in ms",
)
parser.add_argument(
    "--corrstartmjd",
    type=float,
...
if args.freq < 0:
    print(
        "You have to supply a frequency for the snoopy files! Don't be lazy, that's how accidents happen."
    )
    sys.exit()

if args.timediff < -10000:
    print(
        "You have to specify a timediff! It should just be the geometric delay from ASKAP to the geocentre, "
    )
    print(
        "now everything has been fixed.  Don't be lazy, that's how accidents happen."
    )
    sys.exit()
...
if args.corrstartmjd < 0:
    print(
        "You have to specify a corrstartmjd. getGeometricDelay.py will give it to you"
    )
    sys.exit()
...
with open("craftfrb.bin.binconfig", "w") as binconfout:
    ...
    for i in range(numbins + 1):
        ...
        binconfout.write(f"{phasestr}{binstartphase + i*bindeltaphase:.9f}\n")
