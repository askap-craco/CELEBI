...
class VcraftFile:
    ...
    def header_file(self):
        ...
        if Path(hdrfile).is_file():
            ...
            raise OSError(f"Header file '{hdrfile}' does not exist.")
...
class VcraftMux:
    ...
    def __init__(self, vcraft_files, delays=None, default_freq_offset=-1.0):
        ...
        freq_offset = float(
            self.hdr_identical("FREQ_OFFSET", default_freq_offset)
        )
        freqs += freq_offset
        ...
        beam_ra = np.array(
            list(map(float, self.allhdr("BEAM_RA", -99)))
        ).mean()  # TODO: make craft_vdump write same BEAM_RA for all card/fpgas
        beam_dec = np.array(
            list(map(float, self.allhdr("BEAM_DEC", -99)))
        ).mean()
...
def mux_by_none(filenames, **kwargs):
    ...
    all_files.sort(
        key=lambda f: (f.hdr["ANT"][0], f.hdr["CARD_NO"], f.hdr["FPGA_ID"])
    )
    return all_files
