...
parser.add_option(
    "-r", "--refant", type=int, default=3, help="The reference antenna to use."
)
parser.add_option(
    "-u", "--userno", type=int, default=2, help="The AIPS user number"
)
parser.add_option(
    "-s",
    "--sourcename",
...
parser.add_option(
    "--timestep", type=int, default=None, help="Index of time step to image"
)
parser.add_option(
...
allcalxcorplotfilename = os.path.abspath(f"allcalxcor{xpol_prefix}{src}.ps")
...
if not options.targetonly:
    ...
    if not options.skipplot:
        ...
        vlbatasks.plotbandpass(
            caldata, bpversion, plotbptable, plotsperpage, bptableplotfilename
        )
...
if not options.targetonly:
    ...
    vlbatasks.splitmulti(
        caldata, clversion, outklass, options.sourcename, seqno
    )
    vlbatasks.writedata(
...
if not options.calibrateonly:
    ...
    vlbatasks.splitmulti(
        targetdata, clversion, outklass, options.sourcename, seqno
    )
    vlbatasks.writedata(
...
def write_timestep_loop(write_file, fmt_str, vals):
    ...
    i = 0
    start = Time(options.start, format="mjd")
    while (i + 1) * options.inttime < options.duration:
        # get end time by adding inttime to start time
        end = start + options.inttime * un.s
...
def write_single_timestep(write_file, fmt_str, vals, i):
    start = Time(options.start, format="mjd") + options.inttime * un.s * i
    end = start + options.inttime * un.s
    start_str = start.iso.replace(" ", "/").replace("-", "/")
    end_str = end.iso.replace(" ", "/").replace("-", "/")
        ...
        for pol in polarisations:
            ...
            if options.dirtyonly:
                ...
                vals = [
                    ...
                    imsize,
                    pol,
                    options.averagechannels,
                    phasecenter,
                    maskstr,
                    outlierfields,
                    pixelsize,
                ]
                if options.doalltimesteps:
                    write_timestep_loop(casaout, fmt_str, vals)
                elif options.dosingletimestep:
                    write_single_timestep(
                        casaout, fmt_str, vals, options.timestep
                    )
                else:
                    casaout.write(
