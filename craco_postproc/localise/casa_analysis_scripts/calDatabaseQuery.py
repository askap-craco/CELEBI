...
#
#
#$Id: calDatabaseQuery.py,v 1.152 2021/08/24 15:30:50 thunter Exp $
...
#            print "searchMeasurements103 failed.  Searching using the older method searchMeasurements913"
    ...
    def gridSourceReport(self, nsources=None, bands=[3,6,7], sourceType=[64], catalogues=[5]):
        """
        ...
        for src in range(nsources):
            ...
            age[source] = {}
            line = '%11s' % (source)
            m = self.searchFlux(source,returnMostRecent=True,verbose=False, catalogues=catalogues)
            if (m['uvmin'] is None):
                uvminmax = 'None - '
            else:
            ...
            if (m['uvmax'] is None):
                ...
                uvminmax += '%.0fkl' % (m['uvmax'])
            for band in bands:
                age[source][band] = self.searchFlux(source,band=band,verbose=False,catalogues=catalogues)
    ...
    def searchFlux(self, sourcename=None, date='', band = None, fLower=1e9,
        ...
        catalogues: the catalogues parameter for wrapSearch(), 
              default: None == the wrapSearch default 
        ...
        if (measurements == None):
            ...
            measurements = self.wrapSearch(name=sourcename, limit=limit, date=date, fLower=fLower, 
                                           ...
                                           types=types, catalogues=catalogues)
...
#            uniqueDegrees = list(np.unique(tuples))  # this reverses the order of the tuple!
            ...
            if (verbose or sourcenameWildcard):
                ...
                if (maxrows > 0):
                    ...
                    if (showPolarization):
                        print(' Rank | Flux Density (Jy)  | YYYY-MM-DD (age)  | Meas. Freq | catalog  | Source Name    | uvmin-uvmax      | %Pol | Angle |')
                    else:
