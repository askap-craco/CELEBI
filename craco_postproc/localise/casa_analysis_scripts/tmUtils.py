...
$Id: tmUtils.py,v 1.51 2021/08/12 03:02:47 ahirota Exp $
"""
...
def plot_monitor_data_to_png(antenna, device, monitorpoint, \
    ...
    for mp in monitorpointlist:
        ...
        if showplot:
            plf.show()
            import six
            dummy = six.moves.input('Hit return to proceed: ')
        pl.close()
...
def get_weather_data(startdate, enddate, station = 'Meteo1',removefile=False):
    ...
    if (station != 'Meteo1') and (station != 'Meteo2'):
    ...
    for filename in files:
        ...
        for k in list(weatherdata.keys()):
            try:
